...
class CotiTreasuryOverviewPage extends StatefulWidget {
  const CotiTreasuryOverviewPage({super.key});

  @override
  State<CotiTreasuryOverviewPage> createState() => _CotiTreasuryOverviewPageState();
}

class _CotiTreasuryOverviewPageState extends State<CotiTreasuryOverviewPage> {
...
print("url = $url");
      ...
      for (final tx in transactions) {
        if (tx['result'] == 'success' &&
            tx['transaction_types'] != null &&
            tx['transaction_types'].isNotEmpty &&
            tx['transaction_types'][0] == 'coin_transfer') {
          allTransactions.add(tx);
        }
      }

      if (data['next_page_params'] == null) break;
      params = Map<String, dynamic>.from(data['next_page_params']); 
    }
...
Future<String?> fetchZnsAddress(String domainInput) async {
  ...
  Future<void> _fetchData() async {
    ...
    setState(() {
      isLoading = true;
      statusKey = 'cotiforwalletoverview.fetching_status';
      statusArgs = [];
    });
    ...
    if (!isValid) {
      setState(() {
        statusKey = 'cotiforwalletoverview.invalid_address';
        resultText = null;
        deposits.clear();
        withdrawals.clear();
        labels.clear();
    ...
    try {
      ...
      setState(() {
        ...
        statusKey = labels.isNotEmpty
            ? 'cotiforwalletoverview.success_status'
            : 'cotiforwalletoverview.no_tx_found';
        resultText = labels.isNotEmpty
            ? 'cotiforwalletoverview.result_text'
                .tr(args: [
...
Widget buildChart(BuildContext context) {
                      ...
                      touchTooltipData: BarTouchTooltipData(
                        ...
                        getTooltipItem: (group, groupIndex, rod, rodIndex) {
                          ...
                          return BarTooltipItem(
                            '${isDeposit ? tr('cotiforwalletoverview.deposit') : tr('cotiforwalletoverview.withdrawal')}\n$label\n$value',
                            TextStyle(
                              color: textColor,
  ...
  Widget build(BuildContext context) {
    ...
    return MainLayout(
      title: tr('cotiforwalletoverview.treasury_chart_title'),
      child: Container(
        ...
        child: Center(
          child: SingleChildScrollView(
            ...
            Column(
              ...
              children: [
                Text(tr('cotiforwalletoverview.enter_wallet'),
                ...
                if (resultText != null)
                    SelectableText(
                        tr('cotiforwalletoverview.result_text', args: [
                        deposits.reduce((a, b) => a + b).toStringAsFixed(4),
                        withdrawals.reduce((a, b) => a + b).toStringAsFixed(4),
                        ]),
